...
export default function GoalSetter() {
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 'var(--spacing-lg)' }}>
          <div className="card-header">
            <h3 className="card-title">Doel Aanmaken</h3>
          </div>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-md)' }}>
            <div>
              ...
              >
                ...
                <option value="muscle_gain">Spieropbouw</option>
            ...
            <div className="grid grid-2">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="kg, herhalingen, minuten..."
                />
              </div>
            </div>
            <div>
      ...
      <h2 style={{ marginBottom: 'var(--spacing-md)' }}>Actieve Doelen</h2>
            ...
            <div key={goal.id} className="card">
              ...
              {goal.target_value && (
                <>
                  ...
                  <div style={{ marginBottom: 'var(--spacing-sm)' }}>
                    <label>Voortgang Bijwerken</label>
                    <input
                      type="number"
      ...
      {activeGoals.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: 'var(--spacing-lg)', marginBottom: 'var(--spacing-lg)' }}>
          <p style={{ margin: 0, fontWeight: 700 }}>GEEN ACTIEVE DOELEN</p>
        </div>
      )}

      {completedGoals.length > 0 && (
        <>
          <h2 style={{ marginBottom: 'var(--spacing-md)' }}>Voltooide Doelen</h2>
          <div className="grid grid-2">
            {completedGoals.map(goal => (
              <div key={goal.id} className="card" style={{ opacity: 0.7 }}>
                <h4>{goal.title}</h4>
                <p style={{ fontSize: '0.875rem', margin: 0 }}>
                  ✓ Voltooid {goal.completed_at ? new Date(goal.completed_at).toLocaleDateString() : ''}
                </p>
              </div>
            ))}
          </div>
